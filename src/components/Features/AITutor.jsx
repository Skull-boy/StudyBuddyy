            ...
            <div className="relative group">
                ...
                <AnimatePresence>
                    {(isListening || isSpeaking || response) && (
                        <motion.div
                            ...
                            className="absolute bottom-full right-0 mb-4 w-72 bg-white/90 backdrop-blur-xl p-4 rounded-2xl shadow-2xl border border-gray-200 dark:bg-dark-800/90 dark:border-white/10 transition-colors duration-300"
                        >
                            <div className="flex items-start gap-3">
                                <div className="bg-indigo-100 p-2 rounded-full dark:bg-brand-500/20">
                                ...
                                <div className="flex-1">
                                    ...
                                    <p className="text-gray-800 text-sm font-medium leading-relaxed dark:text-gray-200">
                                        {response}
                                    </p>
                                </div>
                                <button
                ...
                <motion.button
                    ...
                    className={`w-16 h-16 rounded-full flex items-center justify-center shadow-lg transition-all duration-300 ${isListening
                        ...
                        : isSpeaking
                            ? 'bg-indigo-600 shadow-indigo-500/30 ring-4 ring-indigo-500/20 dark:bg-brand-600 dark:shadow-brand-500/30 dark:ring-brand-500/20'
                            : 'bg-white text-indigo-600 shadow-xl hover:shadow-indigo-200 border border-gray-100 dark:bg-dark-800 dark:text-brand-400 dark:hover:shadow-brand-500/10 dark:border-white/10'
                        }`}
                >
                    {isListening ? (
                        <MicOff className="w-8 h-8 text-white" />
                    ) : (
                        <Mic className={`w-8 h-8 ${isSpeaking ? 'text-white' : 'text-indigo-600 dark:text-brand-400'}`} />
                    )}
                </motion.button>

                {/* Visualizer Ring when Speaking */}
                {isSpeaking && (
                    <div className="absolute inset-0 rounded-full border-4 border-indigo-400 opacity-50 animate-ping pointer-events-none dark:border-brand-400" />
                )}
            </div>
        </motion.div>
    );
